...
| 3.5 | **Web First** | 모바일 웹 집중 — PWA 문자 공유 신고 + 서비스 소개 | 🔜 진행 중 |
...
## Stage 3.5 — Web First (1차 목표: 모바일 웹)

> **전략:** 모바일 웹에서 먼저 검증하고, 안정화된 기능만 앱으로 반영한다.
> 웹의 역할은 **신고·공유 + 서비스 소개**로 좁힌다 — 수신 감지는 브라우저가 원리상 불가능하므로 앱 고유 기능으로 남긴다.

### 완료

- [x] **PWA 설치 지원** — `manifest.json` + `sw.js` + 아이콘(192/512).
  홈 화면에 설치돼야 문자 앱의 공유 목록에 SpamCut이 나타난다
- [x] **문자 공유로 신고 (Web Share Target)** — 문자 앱에서 `공유 → SpamCut` 시
  본문이 설명란에 자동 입력되고, 본문에서 발신번호 후보를 추출해 채운다.
...
- [x] **개인정보처리방침 페이지** — `backend/public/privacy.html`. 소개·로그인에서 링크. 앱 심사 URL로 재사용
- [ ] **오탐 이의제기** — "이 번호는 스팸이 아니에요". 지인·회사 번호가 RED로 잡혔을 때
  사용자가 쓸 수 있는 수단이 현재 전혀 없다 *(API 추가 — Confirm 대상)*
- [ ] **앱 아이콘 교체** — 현재 아이콘은 스크립트로 생성한 임시 도형. 정식 디자인 필요
- [x] **웹 신고 후 안내 강화** — 접수 후 결과 카드: 즉시 등록되지 않음, 검증 시 Token 10/5 지급
- [x] **번호 표시 포맷 통일** — 조회·이력·지갑·헤더를 `formatPhoneDisplay`로 010-XXXX-XXXX 통일

### 앱 — 웹에서 안정화된 뒤 반영 (2차)

> Play Store 출시 시점에 필요한 항목들. 웹 검증이 끝난 뒤 착수한다.

- [ ] **앱 내 계정 삭제·로그아웃 UI** — `DELETE /api/v1/users/me` 호출부가 없다.
  Play는 계정 생성 앱에 앱 내 삭제 경로를 필수로 요구
- [ ] **`READ_SMS` 권한 제거 검토** — 매니페스트에 선언만 있고 실사용처가 없다.
  Play의 SMS 민감 권한 정책은 선언만으로도 별도 심사를 요구하므로 제거가 유리
  *(매니페스트 변경 — Confirm 대상)*
- [ ] **데이터 세이프티 서식** — 전화번호(서버), 수신 번호·문자 미리보기(기기 로컬 30일), FCM 토큰
- [ ] **권한 사전 고지 화면** — 시스템 다이얼로그 전에 이유를 설명
- [ ] **권한 거부 상태 복구** — 현재 `permissionLauncher` 결과를 무시해서,
  거부하면 앱이 조용히 무력화되고 사용자는 이유를 모른다
- [ ] **CallScreening 기본 앱 설정 유도** — `RoleManager.createRequestRoleIntent(ROLE_CALL_SCREENING)` 호출부 없음
- [ ] **차단 이력 화면** — 자동 거절한 전화 목록. 없으면 "전화가 안 온다"는 불신으로 이어진다
- [ ] **내 허용목록** — 택배·회사 등 자주 오는 번호 경고 제외
- [ ] **앱 내 기능 공백** — 번호 조회·신고 이력·초대·거래 내역 화면 (모두 웹에만 존재).
  `GET /api/v1/reports/my`는 Android API 인터페이스에 아직 정의도 없다
- [ ] **최근 수신 내역 프라이버시 제어** — 미리보기 저장 끄기 / 전체 삭제 / 보관 기간 선택
- [ ] **배터리 최적화 예외 안내** — 국내 제조사 최적화로 백그라운드 수신이 끊기는 문제
- [ ] **국가 코드 하드코딩 해제** — `PhoneNumbers.DEFAULT_COUNTRY_CODE = "82"` 고정
- [ ] **연락처 이름 표시** *(`READ_CONTACTS` 권한 추가 — Confirm 대상)*
